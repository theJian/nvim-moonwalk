...
function M.ciede2000(lab1, lab2)
    ...
    if C1_prime * C2_prime == 0 then
        ...
        if dh_prime > 180 then dh_prime = dh_prime - 360 end
        if dh_prime < -180 then dh_prime = dh_prime + 360 end
    end
    ...
    -- T calculation
    local T = 1 - 0.17 * cos(radians(H_mean - 30)) +
    ...
    local deltaE = sqrt((dL_prime / SL)^2 + (dC_prime / SC)^2 + (dH_prime / SH)^2 + RT * (dC_prime / SC) * (dH_prime / SH))
